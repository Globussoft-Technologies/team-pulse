...
def main():
    args = parse_args()
    # Calendar-day window: --days 1 = yesterday only (00:00 UTC → today 00:00 UTC)
    if args.since and args.until:
        ...
        since = (today - timedelta(days=args.days)).isoformat() + "T00:00:00Z"
    ...
    if args.md:
        ...
        date_label = since[:10]  # YYYY-MM-DD
        now_utc = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
        total_authors = len(author_stats)
        total_commits = sum(s["commits"] for s in author_stats.values())
        total_lines = sum(s["add"]+s["del"] for s in author_stats.values())
        total_repos = len({r for s in author_stats.values() for r in s["repos"]})
        print("<!-- AUTOGENERATED — DO NOT EDIT. See .github/workflows/daily-report.yml -->")
        print()
        print("> [!CAUTION]")
        ...
        print(">")
        print("> This page is regenerated every night at 00:00 UTC. Ship code that lands in a repo, or get visible.")
        print()
        print(f"# Top programmers — {date_label} UTC")
        print()
        print("Real authored code only. Excludes merge commits, vendor code, lockfiles, binaries, minified bundles, and generated files. All branches across the org are scanned (not just `main`), so feature-branch work counts.")
        print()
        print("| # | Engineer | Commits | + | − | Net | Repos |")
        print("|---:|---|---:|---:|---:|---:|---:|")
        for i, (a, s) in enumerate(ranked[:args.top], 1):
            net = s["add"]-s["del"]
            sign = "+" if net >= 0 else ""
            print(f"| {i} | [@{a}](https://github.com/{a}) | {s['commits']} | {s['add']:,} | {s['del']:,} | {sign}{net:,} | {len(s['repos'])} |")
        print()
        print(f"**Total**: {total_authors} engineers · {total_commits} commits · {total_lines:,} lines of authored code across {total_repos} repos.")
        print()
        print("---")
        print()
        print(f"_Last updated: {now_utc}. [How this is calculated](https://github.com/Globussoft-Technologies/.github-private/blob/main/scripts/team_activity.py)._")
        return
